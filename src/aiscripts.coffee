class aiscripts
  ...
  follow: (bumprad, inrad, outrad) ->
    ...
    (scenegraph) ->
      ...
      srcpos = new vect (Math.floor @area.p.x/mapsrc.tilesize.x), (Math.floor @area.p.y/mapsrc.tilesize.y)
      destpos = new vect (Math.floor scenegraph[0].area.p.x/mapsrc.tilesize.x), (Math.floor scenegraph[0].area.p.y/mapsrc.tilesize.y)
      ...
      if inrad < distance and path.length > 0
        ...
        vectdir.x = -1 if vectdir.x is 0 and @area.p.x + @area.s.x > mapsrc.tilesize.x*(srcpos.x + 1) and graph.input[srcpos.x + 1][srcpos.y + vectdir.y] is 1
        vectdir.y = -1 if vectdir.y is 0 and @area.p.y + @area.s.y > mapsrc.tilesize.y*(srcpos.y + 1) and graph.input[srcpos.x + vectdir.x][srcpos.y + 1] is 1
